...
import ModuleTimeAnalytics from "../components/teacher/ModuleTimeAnalytics";
...
const NAV_ITEMS = [
  ...
  { id: "moduleTime",  icon: "⏱", label: "Module Time" },
...
const TeacherDashboard = () => {
  ...
  const [moduleTime,   setModuleTime]   = useState(null);
  ...
  const fetchAnalytics = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const [overviewRes, studentsRes, modulesRes, weakRes, moduleTimeRes] = await Promise.all([
        API.get("/analytics/overview"),
        API.get("/analytics/students"),
        API.get("/analytics/modules"),
        API.get("/analytics/weak-topics"),
        API.get("/analytics/module-time"),
      ]);
      ...
      setModuleTime(moduleTimeRes.data.data || null);
    } catch (err) {
    ...
    <div className="td-root">
      ...
      <div className="td-layout">
        ...
        <main className="td-content">
          ...
          {/* ── Module Time (research analytics) ── */}
          {(activeNav === "overview" || activeNav === "moduleTime") && (
            <section className="td-section-modules">
              <div className="td-section-header">
                <h2 className="td-section-title">⏱ Module Learning Time Analytics</h2>
                {/* <span className="td-section-action"></span> */}
              </div>
              <ModuleTimeAnalytics
                data={moduleTime}
                loading={loading}
                error={!moduleTime && error ? error : null}
              />
            </section>
          )}
