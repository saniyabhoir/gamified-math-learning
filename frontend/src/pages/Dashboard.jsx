...
const HeroStats = ({ modules, progressMap, gameStats }) => {
  ...
  // NOT the game timer. Sum of time spent in each module's learning flow,
  // read from the same local progress cache ModulePage.jsx keeps updated.
  // Purely for display — never affects scoring, XP, or progression.
  const totalModuleTimeSeconds =
    Object.values(progressMap).reduce(
      (sum, p) => sum + (p.moduleTimeAccumulatedMs || 0),
      0
    ) / 1000;
  ...
  return (
    <div className="db-hero-stats">
      ...
      <div className="db-hero-stat">
        <span className="db-hero-stat-val">
          {totalModuleTimeSeconds > 0 ? formatDuration(totalModuleTimeSeconds) : "—"}
        </span>
        <span className="db-hero-stat-lbl">Learning Time</span>
      </div>
    </div>
  );
};
