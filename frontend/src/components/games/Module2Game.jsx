...
// Integrated with GameRegistry + GameMatrics architecture
...
//  Main Component
// ─────────────────────────────────────────────────────────────────────────────
const Module2Game = ({ moduleId, onComplete, onExit }) => {
  // ── Stable session (initialized once, never changes) ─────────────────────
  const [session] = useState(() => buildSession());

  // ── phase: "playing" | "summary"
  // Internal sub-phases: intro | playing | feedback | tip
  ...
  // ── Timer helpers (defined before any useEffect that calls them) ──────────
  ...
  // ── Answer logic — defined before useEffect that references it ────────────
  ...
  // ── Finish game — uses onComplete from GameRegistry architecture ──────────
  const handleFinish = useCallback(() => {
    ...
    const stars          = calcStarsQuiz(correct, TOTAL_QUESTIONS);
    const rewardPoints   = calcRewardPoints(stars, 30);
    const badgeUnlocked  = stars === 3 ? 'Simplification Champion' : null;
    ...
    setSummaryData({ ...result, correct, completionTimeSeconds: completionTime, starsEarned: stars, mistakesByDifficulty: mistakesByDiff });
    setPhase('summary');

    // Call onComplete so GamePage can save result via saveGameResult
    if (typeof onComplete === 'function') {
      setTimeout(() => onComplete(result), 100);
    }
  }, [moduleId, onComplete]);

  // ── Start timer (defined after handleAnswerInternal to avoid hoisting issues) ─
  ...
  }, [stopTimer, handleAnswerInternal]);
  ...
  useEffect(() => {
    ...
    return stopTimer;
  }, [phase, qIndex, startTimer, stopTimer]);
      ...
      <div className="ss-root" role="main">
        ...
        <div className="ss-intro-card">
          ...
          <div className="ss-difficulty-spread" aria-label="Question difficulty split">
            <div className="ss-diff-chip ss-diff-chip--easy"><span>🟢</span><span>4 Easy</span></div>
            <div className="ss-diff-chip ss-diff-chip--medium"><span>🟡</span><span>4 Medium</span></div>
            <div className="ss-diff-chip ss-diff-chip--hard"><span>🔴</span><span>2 Hard</span></div>
          </div>
          <button
          ...
          <button className="ss-back-link" onClick={onExit} aria-label="Back to dashboard">
            ← Back to Dashboard
          </button>
        </div>
      </div>
        ...
        <div className="ss-game-area">
          ...
          >
            <div className="ss-card-meta">
              <span className="ss-chalk-tag" aria-hidden="true">Simplify</span>
              <span
            ...
            <div className="ss-expression-block">
              ...
              <div className="ss-expression" aria-label={`Simplify the expression: ${q.expression}`}>
                {q.expression}
              </div>
            </div>
          ...
          <div className="ss-options-grid" role="group" aria-label="Answer options">
            {q.options.map((opt, i) => {
              ...
              return (
                ...
                >
                  <span className="ss-opt-letter" aria-hidden="true">{OPTION_LABELS[i]}</span>
  ...
  // ─────────────────────────────────────────────────────────────────────────
  //  RENDER: SUMMARY ("summary" phase = external "summary" phase)
  // ─────────────────────────────────────────────────────────────────────────
  if (phase === 'summary' && summaryData) {
    const {
      score, accuracy, correct, starsEarned, rewardPoints,
      badgeUnlocked, completionTimeSeconds, mistakesByDifficulty,
    } = summaryData;
      ...
      <div className="ss-root" role="main">
        ...
        <div className="ss-summary-card" role="region" aria-label="Game summary">
          ...
          <div className="ss-summary-actions">
            ...
            <button className="ss-dash-btn" onClick={onExit} aria-label="Return to dashboard">
              🏠 Dashboard
            </button>
          </div>
